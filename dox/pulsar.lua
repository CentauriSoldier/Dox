...
--[[!
@module dox.pulsar
@func dox.pulsar.writeSnippets
@scope global
@desc Writes pulsar snippets to the snippets file provided based on the lua files which were processed for documentation.
@param pPulsarSnippetsFile string The full path to the pulsar snippets file. If it does not exist, it will be created.
...
function dox.pulsar.writeSnippets(pPulsarSnippetsFile, sSnippetSection)

	--try to process pulsar snippets
	if (type(pPulsarSnippetsFile) == "string" and
		...
		local hSnippets 	= io.open(pPulsarSnippetsFile, "r");
		...
		if (hSnippets) then
			...
			--if the section already exists, prepare to overwrite it
			...
			local hSnippets 	= io.open(pPulsarSnippetsFile, "w");
